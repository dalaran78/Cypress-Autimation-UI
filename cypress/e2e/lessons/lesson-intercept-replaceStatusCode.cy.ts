it('network request spy', function() {
    ...
    // Intercept course request and force 500 error
     cy.intercept(`https://server-stage.pasv.us/course/coursesProgress/${Cypress.env('USERID_LOCAL_STAGE')}`, {
      statusCode: 500
    }).as('course');
    ...
    cy.get('#normal_login_password').type(Cypress.env('PASSWORD_LOCAL_STAGE'), { log: false });
    ...
    cy.visit(`https://stage.pasv.us/profile/${Cypress.env('USERID_LOCAL_STAGE')}/progress`);
