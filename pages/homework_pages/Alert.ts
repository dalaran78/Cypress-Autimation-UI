class Alert {
  ...
  jsConfirmOk() {
    ...
    cy.on("window:confirm", () => true);
    cy.get(this.confirmResult).should(
      "have.text",
      this.afterConfirmText + `${okText}`,
    );
  }
  ...
  jsConfirmCancel() {
    ...
    cy.get(this.confirmResult).should(
      "have.text",
      this.afterConfirmText + `${cancelText}`,
    );
  }
  ...
  jsPromptOk() {
    ...
    cy.on("window:confirm", () => true);
    cy.get(this.promptResult).should(
      "have.text",
      this.afterPromptText + `${promptText}`,
    );
  }
